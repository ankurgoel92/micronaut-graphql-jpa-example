...
public class SecuredInterceptor implements MethodInterceptor<Object, Object>, Ordered {
    ...
    public Object intercept(MethodInvocationContext<Object, Object> context) {
        ...
        context.findAnnotation(Secured.class).ifPresent(annot -> {
            ...
            for (String authority : annot.getValue(String[].class).orElseThrow(RuntimeException::new)) {
                matches = authority.equals(SecurityRule.IS_ANONYMOUS)
                        || (!(authority.equals(SecurityRule.DENY_ALL)) && !(authority.equals(SecurityRule.IS_AUTHENTICATED) && !securityService.isAuthenticated()) && !(!authority.equals(SecurityRule.IS_AUTHENTICATED) && !securityService.hasRole(authority)));
                if (!matches)
            ...
            if (!matches) {
                logger.debug("Security rule rejects: {} for: {}", String.join(",", rejectedAuthorities), context.getTargetMethod());
